...
export function registerCarHandlers() {
  ...
  ipcMain.handle('car:getAll', (): IpcResponse<Car[]> => {
    try {
      const rawCars = db.prepare('SELECT * FROM cars ORDER BY created_at DESC').all()
   ...
   */
  ipcMain.handle('car:getById', (_event: unknown, id: number): IpcResponse<Car> => {
    try {
      // Validate input ID
      if (!Number.isInteger(id) || id <= 0) {
        return { success: false, error: 'Invalid car ID: must be a positive integer' }
      }
   ...
   */
  ipcMain.handle('car:create', (_event: unknown, carData: CarCreateInput): IpcResponse<Car> => {
    try {
      // Validate input data
      ...
      const rawNewCar = db.prepare('SELECT * FROM cars WHERE id = ?').get(info.lastInsertRowid)
   ...
   */
  ipcMain.handle('car:update', (_event: unknown, id: number, carData: CarUpdateInput): IpcResponse<Car> => {
    try {
      // Validate input ID
      if (!Number.isInteger(id) || id <= 0) {
        return { success: false, error: 'Invalid car ID: must be a positive integer' }
      ...
      const updateClause = fieldsToUpdate.map((field) => `${field} = @${field}`).join(', ')
      const stmt = db.prepare(`UPDATE cars SET ${updateClause} WHERE id = @id`)
      stmt.run({ ...validatedData, id })

      const rawUpdatedCar = db.prepare('SELECT * FROM cars WHERE id = ?').get(id)
   ...
   * Delete a car by ID
   */
  ipcMain.handle('car:delete', (_event: unknown, id: number): IpcResponse<void> => {
    try {
      // Validate input ID
      if (!Number.isInteger(id) || id <= 0) {
        return { success: false, error: 'Invalid car ID: must be a positive integer' }
      ...
      const existingCar = db.prepare('SELECT id FROM cars WHERE id = ?').get(id)
      if (!existingCar) {
        return { success: false, error: `Car with ID ${id} not found` }
      }
